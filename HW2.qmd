...
# Extract covariates for treated and control units
...
# Calculate distances for treated unit i=1 to all control units
...
# Calculate distances for treated unit i=2 to all control units
...
# Optimal matching
match_i1 <- which.min(distance_matrix[1,])
match_i2 <- which.min(distance_matrix[2,])

matching_results <- data.frame(
  Matching_Pair = c(1, 2),
  Treated_i = c("i=1", "i=2"),
  Control_j = c(paste0("j=", match_i1), paste0("j=", match_i2))
)
...
Based on my calculations, I found that:
- Treated unit i=1 matches with Control unit j=1 (distance = `r round(dist_i1_j1, 3)`)
- Treated unit i=2 matches with Control unit j=2 (distance = `r round(dist_i2_j2, 3)`)

### b) (5 points) Estimate ACE using matched pairs

```{r ace-matching}
# Calculate ACE using matched pairs
...
Using the matched pairs, I calculated the Average Causal Effect (ACE) as the difference between the mean outcomes of treated and matched control units. The ACE is `r ACE_matching` (in $10k), suggesting that the treatment increases income by $`r ACE_matching * 10`k on average.
...
I calculated the propensity score weights using the formula:
- For treated units: w = 1/e(x)
- For control units: w = 1/(1-e(x))
